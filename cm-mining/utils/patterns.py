        ...
        for u, v, label in graph.edges(data='label'):
            if label in ["source", "target", "general", "specific"]:
                if graph.degree(u) == 1:  # Check if u is a leaf node
                    ...
                    if leaf_node_label in ["gen", "characterization", "comparative", "externalDependence", "material",
                                           "mediation",
    ...
    for i, [index_dict, source_graph] in enumerate(source_graphs):
        ...
        for j, target_graph in enumerate(target_graphs):
            ...
            for match in matches:
                ...
                matched_dict = [{'pattern_support': pattern_support, 'pattern_index': pattern_index, 'model_index': j},
                                matched_graph]
        ...
        if graph_hash not in graph_hashes[composite_key]:
            ...
            if not is_duplicate:
                ...
                unique_graphs.append(
                    [{'pattern_support': pattern_support, 'pattern_index': pattern_index, 'model_index': model_index},
                     G])
