...
def process_genset_cardinalities(graphs):
    ...
    for [index_dict, G] in graphs:
        ...
        nodes_to_remove = [node for node, node_data in list(G.nodes(data=True)) if
                           node_data['label'] in labels_to_remove]
            ...
            if label_ in ["restrictedTo"]:
                ...
                if label_ == "general":
                    if G.nodes[n1]['label'] == "gen":
                        ...
                        H.add_edge(node2, n0, label=G.nodes[n1]['label'], label0=G.nodes[n1]['label0']) if 'label0' in \
                                                                                                           G.nodes[
                                                                                                               n1] else H.add_edge(
                            node2, n0, label=G.nodes[n1]['label'])
                    if G.nodes[n0]['label'] == "gen":
                        node2 = f"Class{n0}"
                        H.add_node(node2, label="class")
                        # H.add_edge(node2, n1, label="gen")
                        H.add_edge(node2, n1, label=G.nodes[n0]['label'], label0=G.nodes[n0]['label0']) if 'label0' in \
                                                                                                           G.nodes[
                                                                                                               n0] else H.add_edge(
                            node2, n1, label=G.nodes[n0]['label'])
                if label_ == "specific":
                    if G.nodes[n1]['label'] == "gen":
                        node2 = f"Class{n1}"
                        H.add_node(node2, label="class")
                        # H.add_edge(n0,node2, label="gen")
                        H.add_edge(n0, node2, label=G.nodes[n1]['label'], label0=G.nodes[n1]['label0']) if 'label0' in \
                                                                                                           G.nodes[
                                                                                                               n1] else H.add_edge(
                            n0, node2, label=G.nodes[n1]['label'])
                    if G.nodes[n0]['label'] == "gen":
                        node2 = f"Class{n0}"
                        H.add_node(node2, label="class")
                        # H.add_edge(n1,node2, label="gen")
                        H.add_edge(n1, node2, label=G.nodes[n0]['label'], label0=G.nodes[n0]['label0']) if 'label0' in \
                                                                                                           G.nodes[
                                                                                                               n0] else H.add_edge(
                            n1, node2, label=G.nodes[n0]['label'])
                if label_ == "target":
                    if G.nodes[n1]['label'] in ["characterization", "comparative", "externalDependence", "material",
                                                "mediation", "componentOf", "memberOf", "subCollectionOf",
                                                "subQuantityOf", "bringsAbout", "creation", "historicalDependence",
                                                "manifestation", "participation", "participational", "termination",
                                                "triggers", "instantiation", "relation"]:
                        ...
                        H.add_edge(n0, node2, label=G.nodes[n1]['label'], label0=G.nodes[n1]['label0']) if 'label0' in \
                                                                                                           G.nodes[
                                                                                                               n1] else H.add_edge(
                            n0, node2, label=G.nodes[n1]['label'])
                    if G.nodes[n0]['label'] in ["characterization", "comparative", "externalDependence", "material",
                                                "mediation", "componentOf", "memberOf", "subCollectionOf",
                                                "subQuantityOf", "bringsAbout", "creation", "historicalDependence",
                                                "manifestation", "participation", "participational", "termination",
                                                "triggers", "instantiation", "relation"]:
