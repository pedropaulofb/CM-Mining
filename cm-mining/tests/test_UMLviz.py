...
def visualize_graphs(graphs, subtitles):
    ...
    for pattern_index in pattern_indices:
        ...
        for model_index, subtitle in enumerate(subtitles):
            ...
            nx.draw(graphs[model_index], pos, with_labels=False, font_size=10, font_weight='bold', node_color='b',
                    alpha=0.2)
            nlabels = {k: f"{d.get('label', '')} {d.get('label0', '')}" for k, d in
                       graphs[model_index].nodes(data=True)}
...
def visualize_graph_patterns(graphs):
    for i, [index, graph] in enumerate(graphs):
        ...
        ax.set_title(f"Pattern {index}", fontsize=7, loc='center', wrap=True, pad=12, color='black',
